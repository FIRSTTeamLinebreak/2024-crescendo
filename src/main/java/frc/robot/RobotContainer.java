...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    m_SwerveDriveCommand = new JoystickDriveCommand(m_swerveDrive,
        () -> m_driverController.getLeftX(), () -> m_driverController.getLeftY() * -1, () -> {
          // Rotation
          ...
          return applyLinearDeadZone(
                     JoystickConstants.joystickDeadZone, m_driverController.getRightX() * -1)
              / (m_driverController.getHID().getLeftBumper()
                      ? JoystickConstants.slowTurningDivisor : 1);
  ...
  public void initTeleop() {
    ...
    m_driverController.b().onTrue(
        new InstantCommand(() -> { m_launcher.setLauncherSpeed(1.0); })
            .withTimeout(0.25)
            .andThen(new InstantCommand(() -> { m_launcher.setControlSpeed(0.3); })));
